...
/* Gabarit « Contact » : H1 italique par glissement (colonnes 1-2), coordonnées par glissement latéral (colonne 3),
...
export function ContactGabarit({ titre, infos, colonne4, id }: { titre: string; infos: ReactNode; colonne4: ReactNode; id?: string }) {
  return (
    <Row className="contact-row" as="section" aria-labelledby={id ? `${id}-titre` : undefined}>
      <Cell span={2}>
        <Reveal as="h1" effect="slide-up" className="titre-hero" id={id ? `${id}-titre` : undefined}>
          {titre}
        </Reveal>
      </Cell>
      <Cell>
